...
pro nsp_test_export_csv_base_success
  ...
  nsp_assert_true, n_elements(fields) eq 24, 'Base export row did not contain the expected 24 columns.'
  ...
  nsp_assert_true, finite(double(fields[14])), 'Base export subsolar latitude column is not finite.'
  nsp_assert_true, finite(double(fields[15])), 'Base export subsolar longitude column is not finite.'
  nsp_assert_true, (fields[16] eq '0') or (fields[16] eq '1'), 'Base export occultation_valid column is not an explicit 0/1 flag.'

  if fields[16] eq '1' then begin
    nsp_assert_true, finite(double(fields[22])), 'Occultation-valid base export should contain a finite tangent radius.'
    nsp_assert_true, finite(double(fields[23])), 'Occultation-valid base export should contain a finite tangent altitude.'
  endif else begin
    nsp_assert_true, strpos(strlowcase(fields[22]), 'nan') ge 0, 'Non-occultation base export should mark tangent radius explicitly as NaN.'
    nsp_assert_true, strpos(strlowcase(fields[23]), 'nan') ge 0, 'Non-occultation base export should mark tangent altitude explicitly as NaN.'
  endelse
end
...
pro nsp_test_export_csv_keplerian_success
  ...
  fields = strsplit(data_line, ',', /extract)
  nsp_assert_true, n_elements(fields) eq 32, 'Keplerian export row did not contain the expected 32 columns.'
  nsp_assert_true, fields[0] eq 'step9_kep', 'Keplerian export case_id column is incorrect.'

  for i = 0L, 7L do begin
    nsp_assert_close, double(fields[24 + i]), keplerian_elements[i], 1D-6, 'Keplerian export column does not match the direct helper element ' + strtrim(i, 2)
  endfor
  nsp_assert_close, double(fields[31]), mars_gm, 1D-6, 'Keplerian export Mars GM column does not match the helper value.'
end
